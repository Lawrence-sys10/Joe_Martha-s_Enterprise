    ...
    <div class="bg-gradient-to-r from-amber-500 to-orange-600 shadow-lg">
        <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center">
                <div>
                    <h2 class="text-2xl font-bold text-white">Edit Supplier</h2>
                    <p class="text-amber-100 text-sm mt-1">Update supplier information</p>
                </div>
                ...
                <div class="p-8 space-y-6">
                            ...
                            <div class="group">
                                ...
                                <div class="relative">
                                    ...
                                    <textarea name="address" rows="2" 
                                              class="pl-10 w-full rounded-xl border-2 border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all duration-200">{{ old('address', $supplier->address) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <h3 class="text-xl font-bold text-gray-800">Contact Person</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="payment_terms" class="w-full rounded-xl border-2 border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-200">
                                    <option value="30" {{ old('payment_terms', $supplier->payment_terms) == 30 ? 'selected' : '' }}>Net 30 days</option>
                                    <option value="15" {{ old('payment_terms', $supplier->payment_terms) == 15 ? 'selected' : '' }}>Net 15 days</option>
                                    <option value="7" {{ old('payment_terms', $supplier->payment_terms) == 7 ? 'selected' : '' }}>Net 7 days</option>
                                    <option value="0" {{ old('payment_terms', $supplier->payment_terms) == 0 ? 'selected' : '' }}>Due on receipt</option>
                                </select>
                            </div>
                            
                            <div class="bg-gray-50 rounded-xl p-3">
                                <label class="block text-sm font-medium text-gray-600 mb-1">Current Balance</label>
                                <p class="text-lg font-bold {{ $supplier->current_balance > 0 ? 'text-red-600' : 'text-green-600' }}">
                                    GHS {{ number_format($supplier->current_balance, 2) }}
                                </p>
                                <p class="text-xs text-gray-500 mt-1">Amount you owe this supplier</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        ...
                        <textarea name="notes" rows="2" 
                                  class="w-full rounded-xl border-2 border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-200">{{ old('notes', $supplier->notes) }}</textarea>
                    </div>
